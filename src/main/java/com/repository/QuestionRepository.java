...
public interface QuestionRepository extends Neo4jRepository<Disease, Long> {
    /**
     * 根据药名 找出药品
     */
    @Query("match(n:Drug)-[:PRODUCTS]-(m:DrugProduct)  where n.name = {name} return m.name")
    List<String> getDrugProduct(@Param("name") String name);
     ...
     * 疾病建议吃什么
     */
    @Query("match(n:Disease)-[:DO_EAT_FOODS]-(m:Food) where n.name = {name} return m.name")
    ...
    List<String> getEasyGetPerson(@Param("name") String name);

    /**
     * 疾病的诱因
     */
    @Query("match(n:Disease) where n.name = {name} return n.cause")
    List<String> getReason(@Param("name") String name);

    /**
     * 疾病传播
     */
    @Query("match(n:Disease)-[:GET_WAYS]-(m:GetWay) where n.name={name} return m.name")
    List<String> getTrans(@Param("name") String name);
    ...
    String getCureTime(@Param("name") String name);

    /**
     * 疾病的治疗方式
     */
    @Query("match(n:Disease)-[:CURE_WAYS]-(m:CureWay) where n.name= {name} return m.name")
    List<String> getCureMethod(@Param("name") String name);
    ...
    List<String> getDiseaseCheck(@Param("name") String name);
