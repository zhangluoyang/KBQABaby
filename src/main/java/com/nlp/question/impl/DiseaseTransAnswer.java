...
public class DiseaseTransAnswer implements Answer {
    ...
    public String answer(List<Nature> natures, QuestionRepository questionRepository) {
        ...
        if (!ObjectUtils.isEmpty(diseases)) {
            List<String> trans = questionRepository.getTrans(diseases.get(0).getWord());
            return Arrays.toString(trans.toArray());
        }
        return null;
    }
}
